...
use constant string_position_fn => phi::allocation
  ->constant(bin q{                     # i cc
    ...
    sget02 sget01 =8     iplus m32set   # [.index=]
    sset01 goto                         # &s })
...
BEGIN
{
  ...
  bin_macros->{psomeof}  = bin q{=0     $atleast_fn call};
  bin_macros->{pmanyof}  = bin q{=1     $atleast_fn call};
}
