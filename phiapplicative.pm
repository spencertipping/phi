...
Let's talk about a few things that need to happen:

1. A type needs to be able to specify a parse continuation for a value
2. Parse continuations need to be precedence-aware
3. Parse results need to know their types

(1) is basically the whole purpose of types anyway, so that's not much of a
problem. (2) and (3) are more involved.

=head3 Precedence awareness
It would make some sense to store the current operator precedence in the parse
state, but that ends up being less flexible than what we can do instead, which
is pass in the current precedence level when we ask for a parse continuation.
Then it's up to the type to build a partial C<alt> parser consisting only of the
operators that are higher-precedence. Otherwise we return from the current
parser and try again with a different precedence request (and possibly a
different type altogether).

...so, strictly speaking, a type is a function that takes an operator and
returns a source parser.

=head3 Parse results and types
OK, this is where we start to run into problems. First, not every parse result
has a well-defined type; for example:

  (random > 0.5 ? "foo" : 10)         # what's the continuation here?

Second, types are first-class values but they aren't always available:

  f t:type = y:t -> y                 # what's the continuation here?

So what do we do about this? Two options:

1. For union types, intersect parse continuations
2. Have a default parse continuation that is easy enough to work with

(1) is insane and subtly broken. Remember that types don't exist at runtime --
so we can't introduce this independency unless values somehow gain knowledge of
their types, which isn't fundamentally wrong but isn't something I want to
design into the language.

(2) is where it's at. This also changes the language culture a little; rather
than having fully open-ended operators, parse continuations can be more of an
exception for DSLs and other special cases. Let's get into the functional
design.


=head2 Types and parse continuations, concretely
TODO: how do types specify new literal syntax?
=cut
