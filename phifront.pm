...
# FIXME: the capture node generated by philang is in the wrong format. It should
# generate an nth-last node against the capture object; then this code would be
# much easier.
#
...
use phi convert_capture_list => l           # cs
  dup, nilp,
  phiabstract::const,                       # const(nil)
  l(i_uncons,                               # cs' c
    mcall"abstract",                        # cs' c.abstract
    swap, convert_capture_list_mut, i_eval, # c.abstract cs''
    swap, pnil, swons, swons,               # [cs'' c.abstract]
    phiabstract::op_cons, i_eval),          # cons-op
  if_;
    ...
    l(pcons(l(120, 1), phiops::op_precedence_type),
      ...
      l(                                    # lhs op
        ...
        l(                                  # state 'sym p
          ...
          rot3l,                            # p sym state
          dup, tail, tail, head,            # p sym state scope
          mcall"child",                     # p sym state child
          stack(0, 2), i_symstr,            # p sym state child symname
          phiparse::str, swons,             # p sym state child argp
          phiabstract::arg, rot3l,          # p sym state argp arg child
          mcall"bind_local",                # p sym state child'
          stack(1, 0, 1, 3),                # p sym state p child' state
          lit 2, lset, i_eval,              # p sym state p childstate
          swap, i_eval,                     # p sym state v|e childstate'|[]
          stack(5, 0, 1),                   # v|e childstate'|[]
          dup, nilp,
          pnil,                             # e []
          l(                                # body childstate'
            dup, lit 2, lget, i_eval,       # body childstate' cscope'
            mcall"capture", rot3r,          # capture body childstate'
            dup, lit 2, lget, i_eval,       # capture body childstate' cscope'
            mcall"parent", lit 2, lset, i_eval, # cap body state'
            rot3l,                          # body state' cap
            convert_capture_list, i_eval,   # body state' cap'
            rot3l, phiabstract::fn, i_eval, # state' fn
            generic_val, i_eval,            # state' fnval
            swap),                          # fnval state'
          if_),                             # sym p f

        rot3l, quote, i_eval, rot3r,        # 'sym p f
        swons, swons),                      # ['sym p f...]
